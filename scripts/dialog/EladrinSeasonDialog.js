...
export class EladrinSeasonDialog extends HandlebarsApplicationMixin(
  ...
  static async #onChangeSeason(event, target) {
    ...
    if (!applied) {
      ui.notifications.info(
        game.i18n.format("ELADRIN.NoImagesForSeason", {
    ...
    await ChatMessage.create({
      content: game.i18n.format("ELADRIN.ChatChange", {
    ...
    ui.notifications.info(
      game.i18n.format("ELADRIN.SeasonChanged", {
        season: getSeasonLabel(seasonId),
      })
    );
  ...
  static async #onSaveSeason(event, target) {
    ...
    ui.notifications.info(
      game.i18n.format("ELADRIN.ImageSaved", {
        season: getSeasonLabel(seasonId),
      })
    );
  ...
  static async #onDeleteSeason(event, target) {
    ...
    const confirmed = await foundry.applications.api.DialogV2.confirm({
      window: { title: game.i18n.format("ELADRIN.DeleteImagesTitle", { season: label }) },
      content: game.i18n.format("ELADRIN.DeleteImagesConfirm", { season: label }),
    });
    if (!confirmed) return;

    await deleteSeasonImages(this.#actor, seasonId);
    ui.notifications.info(
      game.i18n.format("ELADRIN.ImagesDeleted", { season: label })
    );
    this.render();
  }
  ...
  #injectBranding() {
    ...
    if (!header || header.querySelector(".eladrin-branding")) return;
    const brand = document.createElement("a");
    brand.className = "eladrin-branding";
